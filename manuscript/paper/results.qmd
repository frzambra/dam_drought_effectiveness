...
manufacture the result: the 0.54 km upstream-downstream elevation gap, scaled by the control
elevation-transmission sensitivity ($-0.21$ per km), caps the maximum elevation-induced slope
difference at about $0.11$, larger than and opposite in sign to the observed $-0.04$ upstream-minus-
downstream difference, so even a full elevation adjustment leaves no downstream-specific buffering
...
the nulls; at face value it points toward *vulnerability*, not buffering. Masking the barren pixels
and re-estimating on vegetated (irrigated orchard) cells removes both the pre-trend failure
($p$ from $3\times10^{-5}$ to $0.40$) and the positive effect ($+0.048$ to $-0.020$), confirming the
confounding is mechanistic rather than a post-hoc exclusion (Supplementary Fig. S7).
...
the outlier-robust count ($+20$ rights per 100 km²) and the winsorized volume, each with an analytic
interval excluding zero (@fig-forest, @tbl-main). But this is the naive-analysis trap the design
exists to catch: dammed basins are markedly more arid, arid basins carry denser water-rights activity,
and this residual imbalance drives the gap, which does not survive randomization inference
($p_{\text{perm}} = 0.52$ for counts, $0.31$ for volume), lying inside the null that random assignment
within climate and aridity strata already produces (Supplementary Fig. S6). Even on water rights there
is no induced demand attributable to the reservoir once siting is accounted for, though a conventional
matched analysis would have concluded otherwise.
