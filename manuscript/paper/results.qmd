...
baseline; the equivalence margin uses the $0.585$ intent-to-treat baseline), well below the
...
post-hoc orchard stratum relies on MOD16, whose documented bias toward zero over irrigated land,
a product property identified independently of our results, disqualifies
its null; post-hoc elements are flagged there (Supplementary Table S21, Fig. S7).
...
has limited power, detecting only divergence accumulating to about half the control cropland level
by 2009 (the observed trend is a seventh of that): parallel trends exclude large divergence,
smaller divergence remains possible (Supplementary Table S14). The null is metric-invariant: on log cropland share the slope gap is
...
($p_{\text{perm}} = 0.077$, cuenca-clustered interval $[-0.3, 4.1]$). Raw, uncapped volumes ranked give the same verdict: median volume per right is nearly identical
across groups and the design rank contrast is null under cuenca-block permutation
...
decline of both peak and trough. The decline is a level shift at the megadrought onset rather than a secular line (peak 87% of
